...
当前学校页面展示的是“截止当天 00:00”的数据。工作流会在北京时间
**08:17、08:47、09:17** 最多尝试三次；第一次成功发送后，后续补跑会自动跳过。
...
6. GitHub Actions 把更新后的 CSV 和通知状态自动提交回仓库，供下一次绘图及防重复发送使用。
...
- `data/notification-state.json` 只在 Telegram 成功接收日报后更新；定时补跑据此避免重复发送。手动运行不受去重限制。
...
工作流使用 UTC 时间，避免依赖调度器的时区转换；中国不使用夏令时：
...
对应北京时间 08:17、08:47 和 09:17。保留非整点分钟可以降低 GitHub Actions
高峰期延迟概率；任意一次成功后，当天剩余补跑会直接退出。
